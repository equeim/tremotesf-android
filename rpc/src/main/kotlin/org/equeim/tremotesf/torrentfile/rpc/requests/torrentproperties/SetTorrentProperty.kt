...
suspend fun RpcClient.setTorrentIdleSeedingLimit(torrentHashString: String, value: Duration) =
    setTorrentProperty(torrentHashString, "seedIdleLimit", value, MinutesToDurationSerializer)
...
suspend fun RpcClient.setTorrentFilesPriority(
    ...
    setTorrentProperty(
        torrentHashString, when (priority) {
            TorrentFiles.FilePriority.Low -> "priority-low"
            TorrentFiles.FilePriority.Normal -> "priority-normal"
            TorrentFiles.FilePriority.High -> "priority-high"
        }, fileIndices
    )
